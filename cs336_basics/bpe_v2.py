...
def train_bpe(input_path: str | os.PathLike,
    ...
    Added dictionary for storing the location of all the pairs in our pretokenized list of words i.e the index of every word a pair might appear in 
    """
    ...
    while i < num_merges:       
        ...
        delta_count = Counter()
        delta_pos = defaultdict(int)

        words_with_pair: Counter[int] = pair_positions[max_pair]  

        for word_id, num_occurence in words_with_pair.items():
            if num_occurence > 0:
                word = words[word_id]

                ids, delta_count, delta_pos = merge(ids=word, pair=max_pair, 
                                                    num_occurence = num_occurence, 
                                                    local_count_delta=delta_count, 
                                                    delta_pos=delta_pos)

                for pair, count in delta_pos.items():
                    pair_positions[pair][word_id] += count
                delta_pos.clear()
    ...
    while idx < len(ids):
        if idx < len(ids) - 1 and ids[idx] == A and ids[idx + 1] == B:
            ...
            if idx + 1 < len(ids):
                ...
                delta_pos[(B, ids[idx + 1])] -= 1
                delta_pos[(C, ids[idx + 1])] += 1
